...
import { AlertTriangle, Bed, ChevronDown, ChevronUp, Lock, Plus, Search, Wallet, X } from 'lucide-react';
import { useData } from '../context/DataContext';
import { Button, Field, Input, Modal, Select, Textarea } from './ui';
import { DateRangePicker } from './DateRangePicker';
import { OccupancyPicker } from './OccupancyPicker';
import NewGuestModal from './NewGuestModal';
import type { Booking, BookingStatus, Channel, Client, Payment } from '../types';
import { BOOKING_STATUS_LABELS, CHANNEL_LABELS, PAYMENT_METHOD_LABELS } from '../types';
import { parseISO } from 'date-fns';
import {
  brl, cn, extraGuestSurcharge, isActiveBooking, nextReservationNumber, nights,
  planPriceForDate, rangesOverlap, todayISO, uid,
} from '../lib/utils';
...
export default function BookingModal({ open, onClose, booking, defaults }: Props) {
  ...
  useEffect(() => {
    if (!open) return;
    if (booking) {
      // Reservas antigas não têm childrenAges — preenche com 0 (idade desconhecida) só pra manter a contagem.
      const ages = booking.childrenAges ?? Array.from({ length: booking.children ?? 0 }, () => 0);
      setForm({
  ...
  const createGuest = async (data: Omit<Client, 'id'>) => {
    const id = await save('clients', data);
    selectClient(id, data.name);
    setNewGuestOpen(false);
  };
  ...
  const datesReady = Boolean(form.checkIn && form.checkOut && form.checkOut > form.checkIn);
  ...
  /** Soma noite a noite usando o tarifário (preço especial da data > dia da semana > base) + hóspede extra. */
  const stayTotal = (roomId: string, checkIn: string, checkOut: string, planId: string | undefined, adults: number, childrenAges: number[]) => {
    const room = rooms.find((r) => r.id === roomId);
    ...
    const surchargePerNight = room.price ? 0 : extraGuestSurcharge(plan, adults, childrenAges);
    let total = 0;
    for (let d = parseISO(checkIn); format(d, 'yyyy-MM-dd') < checkOut; d = addDays(d, 1)) {
      ...
      total += (p ?? fallback) + surchargePerNight;
    }
    return total;
  };

  // Preço sugerido automático — recalcula também quando muda ocupação (hóspede extra / gratuidade de criança).
  useEffect(() => {
    if (form.priceTouched || !form.roomId || isBlock) return;
    setForm((f) => ({ ...f, totalPrice: stayTotal(f.roomId, f.checkIn, f.checkOut, f.planId, f.adults, f.childrenAges) }));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [form.roomId, form.checkIn, form.checkOut, form.planId, form.adults, form.childrenAges, isBlock, ratePlans]);
  ...
  const totalGuests = form.adults + form.childrenAges.length;
  const overCapacity = Boolean(maxGuests && totalGuests > maxGuests);
  ...
  const guestName = form.clientId ? clients.find((c) => c.id === form.clientId)?.name : undefined;
  ...
  const submit = async () => {
    ...
    if (!isBlock && !form.clientId) return toast.error('Busque um hóspede já cadastrado ou cadastre um novo.');
    ...
    await save('bookings', {
      ...
      roomId: form.roomId,
      clientId: isBlock ? '' : form.clientId,
      checkIn: form.checkIn,
      checkOut: form.checkOut,
      adults: form.adults,
      children: form.childrenAges.length,
    ...
    >
      <div className="grid gap-6 lg:grid-cols-[1fr_300px]">
        ...
        <div className="space-y-5 min-w-0">
          {/* O status da reserva mora no painel "Resumo da reserva", à direita. */}
              ...
              <Field label="Hóspede principal" required>
                <div className="flex items-start gap-2">
                  ...
                  <button
                    ...
                    className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-slate-300 bg-white text-slate-500 transition hover:border-brand-400 hover:text-brand-700 cursor-pointer"
                  >
                    <Plus size={16} />
                  </button>
                </div>
            ...
            <Field label="Quartos disponíveis">
              <div className="relative">
                {datesReady ? (
                  ...
                  <Lock size={15} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-300" />
                )}
                <Input
                  value={roomQuery}
                  disabled={!datesReady}
                  ...
                  placeholder={datesReady ? 'Procurar…' : 'Selecione as datas primeiro'}
                  className="pl-9 disabled:cursor-not-allowed disabled:bg-slate-50 disabled:text-slate-400"
                />
                {roomPickerOpen && datesReady && (
                  <div className="absolute z-20 mt-1.5 max-h-56 w-full overflow-y-auto rounded-xl border border-slate-200 bg-white p-1 shadow-lg">
            ...
            <div>
                    ...
                    <div className="min-w-[180px] flex-1">
                      <Field label="Ocupação">
                        <OccupancyPicker
                          ...
                          childrenAges={form.childrenAges}
                          maxGuests={maxGuests}
                          onChange={({ adults, childrenAges }) => setForm((f) => ({ ...f, adults, childrenAges }))}
                        />
                      </Field>
                    </div>
              ...
              {overCapacity && (
                <p className="mt-2 flex items-start gap-1.5 rounded-xl bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-700">
                  <AlertTriangle size={14} className="mt-0.5 shrink-0" /> Este quarto comporta até {maxGuests} hóspede{maxGuests === 1 ? '' : 's'} — {totalGuests} pode precisar de cama extra ou ajuste na reserva.
                </p>
              )}
              {conflict && (
        ...
        <div className="lg:sticky lg:top-0 lg:self-start">
          <div className="space-y-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex items-center justify-between gap-2">
              ...
              <div className={cn('relative rounded-full text-xs font-bold', STATUS_BADGE[form.status])}>
                <select
                  value={form.status}
                  onChange={(e) => setForm((f) => ({ ...f, status: e.target.value as BookingStatus }))}
                  className="cursor-pointer appearance-none rounded-full bg-transparent py-1 pl-2.5 pr-7 outline-none"
                >
                  {(Object.keys(BOOKING_STATUS_LABELS) as BookingStatus[])
                    .filter((s) => isEdit || ['pre-booking', 'confirmed', 'checked-in', 'blocked'].includes(s))
                    .map((s) => (
                      <option key={s} value={s}>{BOOKING_STATUS_LABELS[s].toUpperCase()}</option>
                    ))}
                </select>
                <ChevronDown size={13} className="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2" />
              </div>
            </div>
